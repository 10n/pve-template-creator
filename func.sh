...
isidtaken() {
    ...
    if grep -q $idtocheck <<< $containerlist; then 
        return
    fi

    vmlist=$(qm list | awk '{print $1}' | tail -n +2)
    if grep -q $idtocheck <<< $vmlist; then 
        return
    fi
...
downloadimage() {
    ...
    if [ ! -e "$(basename $cloudimageurl)" ]; then
        out=$(wget -S --spider --method HEAD $cloudimageurl 2>&1)
        if echo $out | grep -q "Content-Type: application/octet-stream"; then
